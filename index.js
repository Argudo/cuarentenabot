...
bot.on('message', msg=>{
    ...
    else if(msg.content === 'tk' && isReady){
        playAudio('18',msg);
        isReady = true;
        msg.delete();
    }
    else if(msg.content === 'musica maestro' && isReady){
        playAudio('19',msg);
        isReady = true;
        msg.delete();
    }
    else if(msg.content === 'bua' && isReady){
        playAudio('20',msg);
        isReady = true;
        msg.delete();
    }
...
function playAudio(num, msg){
    ...
    switch (num){
        ...
        case '18':
            ...
            msg.member.voice.channel.join()
            .then(connection => {
                const dispatcher = connection.play(require("path").join(__dirname, './tequiero.mp3'));
                dispatcher.on('finish', finish => voiceChannel.leave());
        })
        case '19':
            var voiceChannel = msg.member.voice.channel;
            msg.member.voice.channel.join()
            .then(connection => {
                const dispatcher = connection.play(require("path").join(__dirname, './cancion.mp3'));
                dispatcher.on('finish', finish => voiceChannel.leave());
        })
        case '20':
            var voiceChannel = msg.member.voice.channel;
            msg.member.voice.channel.join()
            .then(connection => {
                const dispatcher = connection.play(require("path").join(__dirname, './bua.mp3'));
                dispatcher.on('finish', finish => voiceChannel.leave());
        })
    }
    isReady = true;
